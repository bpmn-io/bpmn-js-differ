  ...
  var diffpatcher = new DiffPatcher({
    objectHash: function(obj) {
      ...
      if (isAny(obj, UNIQUE_ELEMENTS)) {
        ...
        // Targets identify inputs unless multiple inputs write the same variable.
        hash = obj.$type + '#' + obj.target;
      } else if (is(obj, 'zeebe:Output')) {

        // Sources identify outputs unless multiple outputs read the same expression.
        hash = obj.$type + '#' + obj.source;
      } else if (is(obj, 'zeebe:ExecutionListener')) {

        // Types identify listeners unless multiple listeners use the same type.
